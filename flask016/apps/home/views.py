...
from apps.models import User, Album, Photo, AlbumTag, AlbumFavor, ArticleTag, Article, ArticleFavor, MusicLangTag, \
    ...
    MovieFavor, MovieComment, UserFollow, UserFriend, TimelineItem, ArticleComment, UserMessage
...
def index():
    ...
    # 最佳创作者
    bestCreators = User.query.order_by(User.creator_score.desc()).all()[0:4]
    for creator in bestCreators:
        creator.face_url = photosSet.url(filename=creator.name + '/' + creator.face)
    # 最佳评论员
    bestCommentors = User.query.order_by(User.comments_score.desc()).all()[0:4]
    for commentor in bestCommentors:
        commentor.face_url = photosSet.url(filename=commentor.name + '/' + commentor.face)
    return render_template("home/index.html", recmm_albums=recmm_albums,
                           recmm_articles=recmm_articles, recmd_musics=recmd_musics,
                           recmd_movies=recmd_movies, bestCreators=bestCreators,
                           bestCommentors=bestCommentors)
...
def user_center():
    ...
    timelineitems = TimelineItem.query.filter_by(reciever=user.name). \
        order_by(TimelineItem.addtime.desc()).all()
    timeline_items_str = ''
    timeline_date_str = ''
    for item in timelineitems:
        item_date = datetime.date(item.addtime)
        if timeline_date_str == '':
            timeline_date_str = str(item_date)
            timeline_items_str += get_timeline_date_label(timeline_date_str)
        else:
            if timeline_date_str != str(item_date):
                timeline_date_str = str(item_date)
                timeline_items_str += get_timeline_date_label(timeline_date_str)
        if item.category == 'article_create':
            article = Article.query.filter_by(title=item.content, user_id=user.id).first()
            item_article_create = get_timeline_item_article_create(item, article)
            timeline_items_str += item_article_create
        if item.category == 'album_create':
            album = Album.query.filter_by(title=item.content, user_id=user.id).first()
            item_album_create = get_timeline_item_album_create(item, album)
            timeline_items_str += item_album_create
        if item.category == 'album_favor':
            sender = User.query.filter_by(name=item.sender).first()
            album = Album.query.filter_by(title=item.content, user_id=user.id).first()
            item_album_favor = get_timeline_item_album_favor(item, album, sender)
            timeline_items_str += item_album_favor
        if item.category == 'article_favor':
            sender = User.query.filter_by(name=item.sender).first()
            article = Article.query.filter_by(title=item.content, user_id=user.id).first()
            item_article_favor = get_timeline_item_article_favor(item, article, sender)
            timeline_items_str += item_article_favor
        if item.category == 'user_follow':
            sender = User.query.filter_by(name=item.sender).first()
            item_user_follow = get_timeline_item_user_follow(item, sender)
            timeline_items_str += item_user_follow
        if item.category == 'user_friend':
            sender = User.query.filter_by(name=item.sender).first()
            item_user_friend = get_timeline_item_user_friend(item, sender)
            timeline_items_str += item_user_friend
        if item.category == 'article_comment':
            sender = User.query.filter_by(name=item.sender).first()
            comment = ArticleComment.query.get_or_404(int(item.content))
            article = Article.query.get_or_404(comment.article_id)
            item_article_comment = get_timeline_item_article_comment(
                item, article, comment, sender)
            timeline_items_str += item_article_comment
        if item.category == 'user_message':
            message = UserMessage.query.get_or_404(int(item.content))
            item_user_message = get_timeline_item_user_message(item, message)
            timeline_items_str += item_user_message
    return render_template("home/user_center.html", timeline_items_str=timeline_items_str)


# 获取时间轴的日期标签
def get_timeline_date_label(date_str):
    return """
        <li class="time-label">
          <span class="bg-red">
            %s
...
# 获取时间轴上发表文章的事件或消息的格式化显示字符串
def get_timeline_item_article_create(tl_item, article):
    article_url = url_for('home.article_read', id=article.id)
    return """
            <li>
                <i class="fa fa-book bg-blue"></i>
                <div class="timeline-item">
                    <span class="time"><i class="fa fa-clock-o"></i> %s</span>
                    <h3 class="timeline-header"><a href="#">%s</a> 发表了一篇文章:
                    <a href='%s'>%s</a></h3>
        ...
        """ % (datetime.time(tl_item.addtime), tl_item.reciever, article_url, article.title,
               article.abstract, article_url)


# 获取时间轴上发布相册的事件或消息的格式化显示字符串
def get_timeline_item_album_create(tl_item, album):
    album_url = url_for('home.album_browse', id=album.id)
    imgs_str = ''
    for photo in album.photos[0:min(5, len(album.photos))]:
        photo_url = photosSet.url(filename=album.user.name + '/' + album.title + '/' + photo.thumbname)
        img_str = '<img src="%s" style="margin-right:10px" width="150px" height="100px">' % photo_url
        imgs_str += img_str
    return """
        <li>
            ...
            <div class="timeline-item">
                <span class="time"><i class="fa fa-clock-o"></i> %s </span>
                <h3 class="timeline-header"><a href="#">%s</a>
                 发布了一个新相册: <a href='%s'>%s</a></h3>
                <div class="timeline-body">
                    %s
                </div>
            </div>
        </li>
    """ % (datetime.time(tl_item.addtime), tl_item.reciever, album_url, album.title, imgs_str)


# 获取时间轴上收藏相册的事件或消息的格式化显示模板
def get_timeline_item_album_favor(tl_item, album, sender):
    album_url = url_for('home.album_browse', id=album.id)
    sender_url = url_for('home.user_index', id=sender.id)
    return """
        <li>
            <i class="fa fa-apple bg-aqua"></i>
            <div class="timeline-item">
                <span class="time"><i class="fa fa-clock-o"></i> %s </span>
                <h3 class="timeline-header no-border"><a href="%s">%s</a>
                收藏了你(<a>%s</a>)的相册:<a href="%s">%s</a>
                </h3>
            </div>
        </li>
    """ % (datetime.time(tl_item.addtime), sender_url, tl_item.sender,
           tl_item.reciever, album_url, album.title)


# 获取时间轴上收藏文章的事件或消息的格式化显示模板
def get_timeline_item_article_favor(tl_item, article, sender):
    article_url = url_for('home.article_read', id=article.id)
    sender_url = url_for('home.user_index', id=sender.id)
    return """
        <li>
            ...
            <div class="timeline-item">
                <span class="time"><i class="fa fa-clock-o"></i> %s </span>
                <h3 class="timeline-header no-border"><a href="%s">%s</a>
                收藏了你(<a>%s</a>)的文章:<a href="%s">%s</a>
                </h3>
            </div>
        </li>
    """ % (datetime.time(tl_item.addtime), sender_url, tl_item.sender,
           tl_item.reciever, article_url, article.title)


# 获取时间轴上粉丝关注的事件或消息的格式化显示模板
def get_timeline_item_user_follow(tl_item, sender):
    sender_url = url_for('home.user_index', id=sender.id)
    return """
        <li>
            ...
            <div class="timeline-item">
                <span class="time"><i class="fa fa-clock-o"></i> %s </span>
                <h3 class="timeline-header no-border">粉丝:<a href="%s"> %s</a>
                关注了你(<a>%s</a>)！
                </h3>
            </div>
        </li>
    """ % (datetime.time(tl_item.addtime), sender_url, tl_item.sender, tl_item.reciever)


# 获取时间轴上用户好友请求接受的事件或消息的格式化显示模板
def get_timeline_item_user_friend(tl_item, sender):
    sender_url = url_for('home.user_index', id=sender.id)
    return """
        <li>
            ...
            <div class="timeline-item">
                <span class="time"><i class="fa fa-clock-o"></i> %s </span>
                <h3 class="timeline-header no-border">用户:<a href="%s"> %s</a>
                接受了你(<a>%s</a>)的好友请求！
                </h3>
            </div>
        </li>
...
# 获取时间轴上评论文章的事件或消息的格式化显示模板
def get_timeline_item_article_comment(tl_item, article, comment, sender):
    article_url = url_for('home.article_read', id=article.id)
    sender_url = url_for('home.user_index', id=sender.id)
    return """
        <li>
            ...
            <div class="timeline-item">
                <span class="time"><i class="fa fa-clock-o"></i> %s </span>
                <h3 class="timeline-header"><a href="%s">%s</a>
                评论了你的文章：<a href="%s">%s</a>,内容如下：</h3>
                <div class="timeline-body">
                    %s
                </div>
            </div>
        </li>
    """ % (datetime.time(tl_item.addtime), sender_url, tl_item.sender, article_url,
           article.title, comment.comment)


# 获取时间轴上用户收到的系统消息的事件或消息的格式化显示模板
def get_timeline_item_user_message(tl_item, message):
    if message.category == 'music_comments_del':
        msg_str = '管理员<a>%s</a>删除了你的音乐评论！' % tl_item.sender
    if message.category == 'movie_comments_del':
        msg_str = '管理员<a>%s</a>删除了你的电影评论！' % tl_item.sender
    return """
        <li>
            <i class="fa fa-envelope bg-green"></i>
            <div class="timeline-item">
                <span class="time"><i class="fa fa-clock-o"></i> %s </span>
                <h3 class="timeline-header">%s,评论内容如下：</h3>
                <div class="timeline-body">
                    %s
                </div>
            </div>
        </li>
    """ % (datetime.time(tl_item.addtime), msg_str, message.content)


@home.route('/user/index/<int:id>')
def user_index(id):
    user = User.query.get_or_404(id)
    user.face_url = photosSet.url(filename=user.name + "/" + user.face)
    user_albums = Album.query.filter(Album.user_id == user.id, Album.privacy != 'private') \
        .order_by(Album.clicknum.desc()).all()
    for aitem in user_albums:
        aitem.cover_url = photosSet.url(user.name + '/' + aitem.title + '/' + aitem.cover)
    user_articles = Article.query.filter(Article.user_id == user.id, Article.privacy != 'private') \
        .order_by(Article.clicknum.desc()).all()
    for artitem in user_articles:
        artitem.cover_url = imgsSet.url(filename=artitem.cover)
    # 相似用户推荐
    recmm_users = User.query.filter(User.id != user.id).all()
    for recmm in recmm_users:
        recmm.face_url = photosSet.url(filename=recmm.name + "/" + recmm.face)
    return render_template("home/user_index.html", user=user, user_albums=user_albums,
                           user_articles=user_articles, recmm_users=recmm_users)


@home.route('/user/follow/')
@user_login_req
def user_follow():
    ...
    if act == 'check':
        existed = UserFollow.query.filter(
            UserFollow.user_id == oid, UserFollow.follower_id == uid).first()
        if not existed:
            return jsonify(isfollow=0)
        else:
            return jsonify(isfollow=1)
    if act == 'add':
        follow = UserFollow(user_id=oid, follower_id=uid)
        db.session.add(follow)
        fans = User.query.get_or_404(uid)  # 粉丝
        fans.ouxiangcount += 1
        ouxiang = User.query.get_or_404(oid)  # 偶像
        ouxiang.fanscount += 1
        db.session.add(fans)
        db.session.add(ouxiang)
        # 把用户关注用户的事件记录到时间轴数据表
        timeline_item = TimelineItem(sender=fans.name,
                                     reciever=ouxiang.name,
        ...
        return jsonify(isfollow=1)
    if act == 'del':
        follow = UserFollow.query.filter(
            UserFollow.user_id == oid, UserFollow.follower_id == uid).first()
...
def user_friend():
    ...
    act = request.args.get('act')
    if act == 'check':
        existed = UserFriend.query.filter(UserFriend.from_id == uid,
                                          UserFriend.to_id == oid).first()
        if not existed:
            existed = UserFriend.query.filter(UserFriend.from_id == oid,
                                              UserFriend.to_id == uid).first()
        if not existed:
            return jsonify(isfriend=0)
        else:
            if existed.state == 'accepted':
                return jsonify(isfriend=1)
            else:
                return jsonify(isfriend=2)
    if act == 'add':
        friend_request = UserFriend(from_id=uid, to_id=oid, state='waiting')
        db.session.add(friend_request)
        db.session.commit()
        return jsonify(isfriend=2)
    if act == 'del':
        friend = UserFriend.query.filter_by(
            from_id=uid, to_id=oid, state='accepted').first()
        from_user = User.query.get(uid)
        to_user = User.query.get(oid)
        if not friend:
            friend = UserFriend.query.filter_by(
                from_id=oid, to_id=uid, state='accepted').first()
            from_user = User.query.get(oid)
            to_user = User.query.get(uid)

        from_user.friendcount -= 1
        to_user.friendcount -= 1
    ...
    if act == 'accept':
        friend = UserFriend.query.filter_by(
            from_id=uid, to_id=oid, state='waiting').first()
        friend.state = 'accepted'
        db.session.add(friend)
        from_user = User.query.get_or_404(uid)
        to_user = User.query.get_or_404(oid)
        from_user.friendcount += 1
        to_user.friendcount += 1
        db.session.add(from_user)
        db.session.add(to_user)
        # 把用户好友请求确认的事件记录到时间轴数据表
        timeline_item = TimelineItem(sender=to_user.name,
                                     reciever=from_user.name,
                                     category='user_friend',
                                     content='接受了好友请求！')
        db.session.add(timeline_item)
        db.session.commit()
        return redirect(url_for('home.user_friends', page=1))
    if act == 'reject':
        friend = UserFriend.query.filter_by(
            from_id=uid, to_id=oid, state='waiting').first()
...
@home.route('/user/fans/<int:page>', methods=["GET"])
@user_login_req
def user_fans(page):
    followers = UserFollow.query.filter_by(user_id=int(session.get('user_id'))) \
        .paginate(page=page, per_page=10)
    fanses = []
    for item in followers.items:
        fans = User.query.get_or_404(item.follower_id)
        fans.face_url = photosSet.url(filename=fans.name + '/' + fans.face)
        fanses.append(fans)
    return render_template("home/user_fans.html", fanses=fanses)


@home.route('/user/ouxiangs/<int:page>', methods=["GET"])
@user_login_req
def user_ouxiangs(page):
    ouxiangs = UserFollow.query.filter_by(follower_id=int(session.get('user_id'))) \
        .paginate(page=page, per_page=10)
    oxes = []
    for item in ouxiangs.items:
        ox = User.query.get_or_404(item.user_id)
        ox.face_url = photosSet.url(filename=ox.name + '/' + ox.face)
        oxes.append(ox)
    return render_template("home/user_ouxiangs.html", oxes=oxes)


@home.route('/user/friends/<int:page>', methods=["GET"])
@user_login_req
def user_friends(page):
    # 获取收到的还未回复的好友请求
    recieved_requests = UserFriend.query.filter(
        UserFriend.to_id == int(session.get('user_id')), UserFriend.state == 'waiting').all()
    # 获得发送方列表
    request_senders = []
    for item in recieved_requests:
        request_sender = User.query.get_or_404(item.from_id)
        request_senders.append(request_sender)
    # 获得好友列表
    friends = []
    friends1 = UserFriend.query.filter_by(from_id=int(session.get('user_id')),
                                          state='accepted').all()
    for f1 in friends1:
        friend = User.query.get_or_404(f1.to_id)
        friends.append(friend)
    friends2 = UserFriend.query.filter_by(to_id=int(session.get('user_id')),
                                          state='accepted').all()
    for f2 in friends2:
        friend = User.query.get_or_404(f2.from_id)
        friends.append(friend)
    for f in friends:
        f.face_url = photosSet.url(filename=f.name + '/' + f.face)
    return render_template("home/user_friends.html", recieved_requests=recieved_requests,
                           request_senders=request_senders, friends=friends)
...
def album_index():
    ...
    # 最佳相册创作者
    bestCreators = User.query.order_by(User.albums_score.desc()).all()[0:4]
    for creator in bestCreators:
        creator.face_url = photosSet.url(filename=creator.name + '/' + creator.face)
...
def album_create():
    ...
    if form.validate_on_submit():
        ...
        # 把用户发布相册的事件记录到时间轴数据表
        timeline_item = TimelineItem(sender=session.get("user_name"),
                                     reciever=session.get("user_name"),
                                     category='album_create',
                                     content=album.title)
        db.session.add(timeline_item)
        db.session.commit()
...
def album_browse(id):
    ...
    # 增加对应相册的浏览量
    album.clicknum += 1
    album.user.albums_score += 1
    album.user.creator_score += 1
    db.session.add(album)
    db.session.commit()
    # 查询推荐相册
    ...
    if act == 'add':
        ...
        if album.user_id == session.get('user_id'):
            ...
            if existedCount >= 1:
                ...
                # 把用户收藏相册的事件记录到时间轴数据表
                timeline_item = TimelineItem(sender=session.get("user_name"),
                                             reciever=album.user.name,
                                             category='album_favor',
                                             content=album.title)
                db.session.add(timeline_item)
                db.session.commit()
    if act == 'del':
...
def article_index():
    ...
    # 最佳文章创作者
    bestCreators = User.query.order_by(User.articles_score.desc()).all()[0:4]
    for creator in bestCreators:
        creator.face_url = photosSet.url(filename=creator.name + '/' + creator.face)
...
def article_create():
    ...
    if form.validate_on_submit():
        ...
        # 把用户发表文章的事件记录到时间轴数据表
        tlitem = TimelineItem(sender=session.get("user_name"),
                              reciever=session.get("user_name"),
                              category='article_create',
                              content=article.title)
        db.session.add(tlitem)
        db.session.commit()
...
def article_read(id):
    ...
    article.clicknum += 1
    article.user.articles_score += 1
    article.user.creator_score += 1
    db.session.add(article)
    db.session.commit()
    ...
    for item in recommed__articles:
        item.coverimgurl = photosSet.url(filename=item.cover)
    # 获取评论列表
    article_comments = ArticleComment.query.filter_by(article_id=article.id). \
        order_by(ArticleComment.addtime.desc()).all()
    for mcmmt in article_comments:
    ...
    if act == 'add':
        ...
        if article.user_id == session.get('user_id'):
            ...
            if existedCount >= 1:
                ...
                # 把用户收藏文章的事件记录到时间轴数据表
                timeline_item = TimelineItem(sender=session.get("user_name"),
                                             reciever=article.user.name,
                                             category='article_favor',
                                             content=article.title)
                db.session.add(timeline_item)
                db.session.commit()
...
@home.route('/article/comment/', methods=["GET"])
def article_comment():
    aid = request.args.get('aid')
    uid = request.args.get('uid')
    content = request.args.get('content')
    comment = ArticleComment(user_id=int(uid), article_id=int(aid), comment=content)
    article = Article.query.get_or_404(int(aid))
    article.commtnum += 1
    user = User.query.get_or_404(int(uid))
    user.comments_score += 1
    db.session.add(user)
    db.session.add(comment)
    db.session.add(article)
    db.session.commit()
    # 把用户发表文章评论的事件记录到时间轴数据表
    tlitem = TimelineItem(sender=session.get("user_name"),
                          reciever=article.user.name,
                          category='article_comment',
                          content=str(comment.id))
    db.session.add(tlitem)
    db.session.commit()
    # 实现评论列表的局部刷新
